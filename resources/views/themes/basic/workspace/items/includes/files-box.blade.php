    ...
    <div class="card-v-body p-4">
            ...
            @foreach ($uploadedFiles as $uploadedFile)
                <div class="uploaded-file uploaded-file-{{ hash_encode($uploadedFile->id) }}">
                    ...
                    <button class="uploaded-file-remove" data-id="{{ hash_encode($uploadedFile->id) }}"
                        data-delete-link="{{ route('workspace.items.files.delete', hash_encode($uploadedFile->id)) }}">
                        ...
                        <div>
                            ...
                            <p class="text-muted mb-0 small">
                                {{ translate(
                                    ...
                                    [
                                        'types' => strtoupper(allowedFileTypes()),
        ...
        <div class="row g-3">
            <div class="col-12">
                ...
                <div class="form-text">
                    {{ translate(':dimensions preview image (.JPG or .PNG)', ['dimensions' => @$settings->item->preview_image_width . 'x' . @$settings->item->preview_image_height]) }}
                </div>
            </div>
            @if ($settings->item->external_file_link_option)
                <div class="col-12">
                    ...
                    <div class="form-text main-file-source-1">
                        @php
                            $fileTypesArray = explode(',', @$settings->item->main_file_types);
            ...
            <div class="col-12">
                ...
                <select name="screenshots[]" class="selectpicker-md selectpicker item-files-select" title="--"
                    multiple>
                    @foreach ($uploadedFiles as $uploadedFile)
...
@push('top_scripts')
    ...
    <script>
        ...
        const uploadConfig = {!! json_encode([
            'upload_url' => route('workspace.items.upload'),
